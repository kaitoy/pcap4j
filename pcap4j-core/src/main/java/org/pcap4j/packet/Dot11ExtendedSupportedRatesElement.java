 ...
 * </pre>
 *
 * The Extended Supported Rates element specifies the rates in the OperationalRateSet parameter
 * and zero or more BSS membership selector values that are not carried in the Supported Rates
 * element. The Information field is encoded as 1 to 255 octets where each octet describes a single
 * supported rate or BSS membership selector.
 ...
 * appropriate value. Rates not contained in the BSSBasicRateSet parameter are encoded with the MSB
 * set to 0, and bits 6 to 0 are set to the appropriate value from the valid range.
 * The MSB of each octet in the Extended Supported Rate element in other management frame types is
 * ignored by receiving STAs.
 * Within Beacon, Probe Response, Association Response, Reassociation Response, Mesh Peering Open,
 ...
 * 6 to 0 are set to the encoded value for the selector.
 *
...
public final class Dot11ExtendedSupportedRatesElement extends Dot11AbstractSupportedRatesElement {
   ...
   * @throws IllegalRawDataException if parsing the raw data fails.
   */
  private Dot11ExtendedSupportedRatesElement(
    byte[] rawData, int offset, int length
  ) throws IllegalRawDataException {
  ...
  @Override
  public String getElementName() { return "Extended Supported Rates"; }
