...
public final class SnapPacket extends AbstractPacket {
   ...
   */
  public static SnapPacket newPacket(
    ...
    if (payloadLength > 0) {
      this.payload
        = PacketFactories.getFactory(Packet.class, EtherType.class)
            .newInstance(rawData, offset + header.length(), payloadLength, header.getProtocolId());
    }
    else {
      this.payload = null;
    }
  ...
  private SnapPacket(Builder builder) {
    if (
         builder == null
      || builder.oui == null
      || builder.protocolId == null
    ) {
      StringBuilder sb = new StringBuilder();
      sb.append("builder: ").append(builder)
        .append(" builder.oui: ").append(builder.oui)
        .append(" builder.protocolId: ").append(builder.protocolId);
  ...
  public static final class SnapHeader extends AbstractHeader {
    ...
    private static final int OUI_OFFSET
      = 0;
    private static final int OUI_SIZE
      = 3;
    private static final int PROTOCOL_ID_OFFSET
      = OUI_OFFSET + OUI_SIZE;
    private static final int PROTOCOL_ID_SIZE
      = SHORT_SIZE_IN_BYTES;
    private static final int SNAP_HEADER_SIZE
      = PROTOCOL_ID_OFFSET + PROTOCOL_ID_SIZE;
